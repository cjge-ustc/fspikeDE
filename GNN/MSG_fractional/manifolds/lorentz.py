...
class Lorentz(geoopt.Lorentz):
    ...
    def geodesic(self, t, x, y):
        ...
        v = self.logmap(x, y)
        return cosh(nomin * t) * x + k_sqrt * sinh(nomin * t) * v / self.norm(
            v, keepdim=True
        )
    ...
    def jacobian_expmap_v(self, x, v):
        ...
        first_term = sinh_div(v_norm).unsqueeze(-1) * (
            eye + x.unsqueeze(-1) @ v_hat.unsqueeze(1)
        )
        second_term = (
            (cosh_div_square(v_norm) - sinh_div_cube(v_norm)).unsqueeze(-1)
            * v.unsqueeze(-1)
            @ v_hat.unsqueeze(1)
        )
...
if __name__ == "__main__":
    ...
    x, v = torch.tensor([[2.0, 1.0, 1.0, 1.0]]), torch.tensor([[1.0, 0.0, 0.0, 2.0]])
