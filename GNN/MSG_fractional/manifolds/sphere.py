...
class Sphere(geoopt.Sphere):
    ...
    def geodesic(self, t, x, y):
        return geodesic(t, x, y, k=torch.tensor([1.0]).to(x.device))
    ...
    def norm(
        self, u: torch.Tensor, x: torch.Tensor = None, *, keepdim=False
    ) -> torch.Tensor:
    ...
    def jacobian_expmap_v(self, x, v):
        ...
        first_term = sin_div(v_norm).unsqueeze(-1) * (
            eye - x.unsqueeze(-1) @ v.unsqueeze(1)
        )
        second_term = (
            (cos_div_square(v_norm) - sin_div_cube(v_norm)).unsqueeze(-1)
            * v.unsqueeze(-1)
            @ v.unsqueeze(1)
        )
...
if __name__ == "__main__":
    ...
    x, v = torch.tensor([[1.0, 0.0, 0.0]]), torch.tensor([[0.0, 0.0, 2.0]])
