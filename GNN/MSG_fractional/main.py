...
parser = argparse.ArgumentParser(
    description="Spiking Graph Neural Networks on Riemannian Manifold"
)
...
parser.add_argument(
    ...
    default="IF",
    choices=["IF", "LIF"],
    help="Which neuron to use",
)
parser.add_argument("--T", type=int, default=10, help="latency of neuron")
parser.add_argument("--n_layers", type=int, default=1)
parser.add_argument(
    "--embed_dim", type=int, nargs="+", default=[32], help="embedding dimension"
)
parser.add_argument(
    "--step_size", type=float, default=0.01, help="step size for tangent vector"
)
parser.add_argument(
    "--v_threshold", type=float, default=1.0, help="threshold for neuron"
)
...
parser.add_argument(
    ...
    type=str,
    default="gl",
    choices=["gl", "predictor"],
    help="Which fde solver to use",
)

configs = parser.parse_args()
results_dir = f"./results/logs"
log_path = (
    f"{results_dir}/{configs.task}_{list2str(configs.manifold)}_{configs.dataset}.log"
)
