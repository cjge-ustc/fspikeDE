...
class Exp:
    ...
    def train(self):
        ...
        for exp_iter in range(self.configs.exp_iters):
            ...
            model = RiemannianSpikeGNN(
                ...
                neuron=self.configs.neuron,
                embed_dim=sum(self.configs.embed_dim),
                n_classes=data["num_classes"],
                step_size=self.configs.step_size,
                v_threshold=self.configs.v_threshold,
                dropout=self.configs.dropout,
                task=self.configs.task,
                use_MS=self.configs.use_MS,
            ...
            if self.configs.task == "NC":
                ...
                f.close()
                best_val, test_acc, test_weighted_f1, test_macro_f1 = self.train_cls(
                    data, model, logger, exp_iter
                )
                energy, params = calc_params(model, data)
    ...
    def cal_cls_loss(self, model, data, mask):
        ...
        weighted_f1, macro_f1 = cal_F1(
            infer.detach().cpu(), data["labels"][mask].detach().cpu()
        )
    ...
    def train_cls(self, data, model_cls, logger, exp_iter: int):
        ...
        for epoch in range(1, self.configs.epochs_cls + 1):
            ...
            loss, acc, weighted_f1, macro_f1 = self.cal_cls_loss(
                model_cls, data, data["train_mask"]
            )
            ...
            logger.info(
                f"Epoch {epoch}: train_loss={loss.item()}, train_accuracy={acc}"
            ...
            if epoch % self.configs.eval_freq == 0:
                model_cls.eval()
                val_loss, acc, weighted_f1, macro_f1 = self.cal_cls_loss(
                    model_cls, data, data["val_mask"]
                )
                logger.info(
                    f"Epoch {epoch}: val_loss={val_loss.item()}, val_accuracy={acc}"
                )
                if acc > best_acc:
                    best_acc = acc
                early_stop(
                    ...
                    model_cls,
                    save=False,
                    path=f"{self.configs.task}_{self.configs.dataset}_{list2str(self.configs.manifold)}_{exp_iter}.pt",
                )
        ...
        np.save(
            file=f"./results/times/backward_time_"
        ...
        with open("time.txt", "a") as f:
            f.write(time_str)
        f.close()
        test_loss, test_acc, test_weighted_f1, test_macro_f1 = self.cal_cls_loss(
            model_cls, data, data["test_mask"]
        )
    ...
    def cal_lp_loss(self, embeddings, infer, model, pos_edges, neg_edges):
        ...
        pos_scores = torch.cosine_similarity(
            infer[pos_edges[0]], infer[pos_edges[1]], -1
        )
        neg_scores = torch.cosine_similarity(
            infer[neg_edges[0]], infer[neg_edges[1]], -1
        )
        preds = list(pos_scores.detach().cpu().numpy()) + list(
    ...
    def train_lp(self, data, model, logger, exp_iter: int):
        ...
        for epoch in range(1, self.configs.epochs_lp + 1):
            ...
            neg_edge_train = data["neg_edges_train"][
                ...
                np.random.randint(
                    0,
                    data["neg_edges_train"].shape[1],
                    data["pos_edges_train"].shape[1],
                ),
            ]
            loss, auc, ap = self.cal_lp_loss(
                embeddings, infer, model, data["pos_edges_train"], neg_edge_train
            )
            loss.backward()
            optimizer_lp.step()
            logger.info(
            ...
            if epoch % self.configs.eval_freq == 0:
                model.eval()
                val_loss, auc, ap = self.cal_lp_loss(
                    embeddings,
                    infer,
                    model,
                    data["pos_edges_val"],
                ...
                logger.info(
                    f"Epoch {epoch}: val_loss={val_loss.item()}, val_AUC={auc}, val_AP={ap}"
                )
                if ap > best_ap:
                    best_ap = ap
                early_stop(
        ...
        with open("time.txt", "a") as f:
            f.write(time_str)
        f.close()
        test_loss, test_auc, test_ap = self.cal_lp_loss(
            embeddings, infer, model, data["pos_edges_test"], data["neg_edges_test"]
        )
